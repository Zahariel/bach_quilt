...
ROW_HEIGHT = max(height + top for height, tops in NOTE_TOP.items() for top in tops.values())
...
class Note(Drawable):
    ...
    def draw(self, rows:typing.Sequence[MusicRow], x, row, *, scale=DEFAULT_SCALE, measure_border=None, **kwargs):
        if self.pitch != "R":
            ...
            if measure_border:
                rows[row].group.append(svg.Text(self.pitch + str(self.octave), 0.5*scale, x, -0.15))
    ...
    def top_y(self):
        ...
        return NOTE_TOP[self.height][self.pitch, self.octave]

    def bottom_y(self):
        if self.pitch == "R": return self.height
        return NOTE_TOP[self.height][self.pitch, self.octave] + self.height
...
class Trill(Note):
    ...
    def top_y(self):
        return NOTE_TOP[self.height][self.first, self.octave + 1 if self.octave_adj else self.octave]
...
class MusicBlock(Drawable):
    ...
    def bottom_y(self):
        return max(note.bottom_y() for note in self.notes)

    def top_y(self):
        return min(note.top_y() for note in self.notes)
